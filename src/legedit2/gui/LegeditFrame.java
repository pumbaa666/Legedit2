...
public class LegeditFrame extends JFrame
{
	...
	public void selectCardForEdit(Card c)
	{
		if(editorMode.equals(EDITOR_MODE.TABBED))
		{
			...
			{
				if(tabs.getComponentAt(i) instanceof CardEditorPanel && ((CardEditorPanel)tabs.getComponentAt(i)).getSelectedCard().equals(c))
				{
	...
	public void selectCardForEdit(CardType c)
	{
		if(editorMode.equals(EDITOR_MODE.TABBED))
		{
			...
			for(int i = 0; i < tabs.getTabCount(); i++)
			{
				if(tabs.getComponentAt(i) instanceof CardEditorPanel && ((CardEditorPanel)tabs.getComponentAt(i)).getSelectedCard().equals(c.getCopy())) // Probablement toujours faux (comparer un Card à un CardType n'a pas de sens)
				{
					tabs.setSelectedIndex(i);
					found = true;
					break;
