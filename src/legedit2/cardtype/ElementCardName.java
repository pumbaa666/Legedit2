...
public class ElementCardName extends CustomElement implements Cloneable
{
	...
	public void drawElement(Graphics2D g)
	{
		if(getValue() != null)
		{
			...
			String defaultFontName = "Percolator.otf";
			Font font = null;
			if(fontName != null)
				defaultFontName = fontName;
			try
			{
				font = Font.createFont(Font.TRUETYPE_FONT, new File("legedit" + File.separator + "fonts" + File.separator + defaultFontName));
//				font = Font.createFont(Font.TRUETYPE_FONT, new File("legedit" + File.separator + "fonts" + File.separator + "Percolator.otf"));
//	    		font = Font.createFont(Font.TRUETYPE_FONT, new File("legedit" + File.separator + "fonts" + File.separator + "MagicSchoolOne.ttf")); // Pumbaa
			...
			int bannerStart = (yModified - (int)lm.getAscent() + (int)lm.getDescent())
					- getPercentage(getPercentage(CustomCardMaker.cardHeight, getScale()), 0.01d);
			...
			*/
			...
			if(includeSubname)
			{
				...
				try
				{
					fontSubname = Font.createFont(Font.TRUETYPE_FONT,
				...
				fontSubname = fontSubname.deriveFont((float)getPercentage(subnameSize, getScale()));
				...
				bannerEnd = yModifiedSubname
						+ getPercentage(getPercentage(CustomCardMaker.cardHeight, getScale()), 0.015d);
			}
			if(highlight.equals(HIGHLIGHT.BLUR) || highlight.equals(HIGHLIGHT.BANNER_BLUR))
			{
				...
				drawUnderlay(bi, g2, BufferedImage.TYPE_INT_ARGB, 0, 0, getPercentage(blurRadius, getScale()),
						blurDouble, getPercentage(blurExpand, getScale()), highlightColour);
			}
			if(highlight.equals(HIGHLIGHT.BANNER) || highlight.equals(HIGHLIGHT.BANNER_BLUR))
			{
				...
				g3.fillRect((getPercentage(CustomCardMaker.cardWidth, getScale()) / 2),
						bannerStart - getPercentage(getPercentage(CustomCardMaker.cardHeight, getScale()), 0.005d),
						getPercentage(getPercentage(CustomCardMaker.cardWidth, getScale()), 0.15d), bannerHeight);
	...
	private String getSubnameValueForDraw()
	{
		...
		{
			return getSubnamePrefix().toUpperCase() + getSubnameValue().toUpperCase()
					+ getSubnameSuffix().toUpperCase();
		}
	...
	{
		String str = "";
		str += "<cardname name=\"" + replaceNonXMLCharacters(name) + "\" " + "value=\""
				+ replaceNonXMLCharacters(getValue()) + "\" " + "subnameValue=\""
				+ replaceNonXMLCharacters(getSubnameValue()) + "\" "
				+ (fontName == null ? " " : "fontname=\"" + replaceNonXMLCharacters(fontName) + "\" ") + "textsize=\""
				+ textSize + "\" " + "subnamesize=\"" + subnameSize + "\" />\n";
		return str;
	}
	...
	public void loadValues(Node node, Card card)
	{
		...
		{
			if(card.getTemplate() != null && card.getTemplate().findElementByType(ElementCardName.class) != null
