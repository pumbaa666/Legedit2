...
//	private int textRotationAngle = 0;
//
//	public void setTextRotationAngle(int angle)
//	{
...
//	}
	...
	public void drawElement(Graphics2D g)
	{
		if(getValue() == null)
			return;
		BufferedImage bi = new BufferedImage(getPercentage(CustomCardMaker.cardWidth, getScale()),
...
//			Font font = Font.createFont(Font.TRUETYPE_FONT, new File("legedit" + File.separator + "fonts" + File.separator + "MagicSchoolOne.ttf"));
			...
			g2.setFont(font);
			g2 = setGraphicsHints(g2);
//			Font fontBold = Font.createFont(Font.TRUETYPE_FONT,
//					new File("legedit" + File.separator + "fonts" + File.separator + "Swiss 721 Black Condensed.ttf"));
...
//				fontBold = new Font(fontNameBold, Font.BOLD, (int)Math.floor(getPercentage(textSizeBold, getScale())));
			Font fontHeader = Font.createFont(Font.TRUETYPE_FONT,
					new File("legedit" + File.separator + "fonts" + File.separator + "Percolator.otf"));
			...
			for(String sectionString:sections)
			{
				...
				if(headerStr != null && !headerStr.isEmpty())
				{
					...
					drawHeader(g2, headerStr.toUpperCase(), fontHeader, /*card.cardType.getBgColor()*/ Color.red, y,
							headerHeight, getPercentage(CustomCardMaker.cardWidth, 0.2d), headerIcon);
				...
				for(WordDefinition wd:words)
				{
					...
					if(s.startsWith("<k>")) // Bold
					{
//						g2.setFont(fontBold); // code original, utilise une font spéciale pour le Bold
						
						// Pumbaa
						Font myFontBold = font.deriveFont(Font.BOLD);
						g2.setFont(myFontBold);						
						metrics = g2.getFontMetrics(myFontBold);
						s = s.replace("<k>", "");
						g2 = setGraphicsHints(g2);
						continue;
					}

					if(s.startsWith("<i>")) // Italique (Pumbaa)
					{
						Font fontItalic = font.deriveFont(Font.ITALIC);
						g2.setFont(fontItalic);
						metrics = g2.getFontMetrics(fontItalic);
						s = s.replace("<i>", "");
						g2 = setGraphicsHints(g2);
						continue;
					}

					if(s.startsWith("<r>")) // Regular
					{
						g2.setFont(font);
					...
					else if(icon == null)
					{
						...
						if(!getPolygon().contains(x + stringLength, y))
						{
							...
							*/
							y += g2.getFontMetrics(font).getHeight()
									+ getPercentage(g2.getFontMetrics(font).getHeight(), textDefaultGapHeight);
							x = getXStart(y);
						}
						g2.drawString(s + " ", x, y);
					...
					else if(icon != null)
					{
						...
						if(icon.isUnderlayMinimized())
						{
							drawUnderlay(i, g2, BufferedImage.TYPE_INT_ARGB, x, modifiedY, textIconBlurRadius,
									textIconBlurDouble, expandTextIcon, Color.black);
						}
			...
			HeaderIcon headerIcon)
	{
		...
		if(headerIcon != null && headerIcon.icon != null)
		{
			...
			int iconx = getPercentage(CustomCardMaker.cardWidth, getScale())
					- getPercentage(getPercentage(CustomCardMaker.cardWidth, getScale()), 0.09d) - bi.getWidth()
					+ (bi.getWidth() / 2);
			int icony = y + (height / 2) - (bi.getWidth() / 2);
			g2.drawImage(bi, iconx, icony, null);
		}
		BufferedImage bi2 = new BufferedImage(getPercentage(CustomCardMaker.cardWidth, getScale()),
				getPercentage(CustomCardMaker.cardHeight, getScale()), BufferedImage.TYPE_INT_ARGB);
		Graphics2D g3 = getGraphics(bi2);
		g3.setColor(Color.WHITE);
		g3.setFont(font);
		g3 = setGraphicsHints(g3);
		g3.drawString(header, getPercentage(getPercentage(CustomCardMaker.cardWidth, getScale()), 0.04d),
				y + g.getFontMetrics(font).getHeight() - (g.getFontMetrics(font).getHeight() / 6));
		if(headerIcon != null && headerIcon.value != null)
		{
			...
			int stringLength = SwingUtilities.computeStringWidth(g.getFontMetrics(font),
					headerIcon.value.toUpperCase());
			g3.drawString(headerIcon.value,
					getPercentage(CustomCardMaker.cardWidth, getScale())
							- getPercentage(getPercentage(CustomCardMaker.cardWidth, getScale()), 0.09d) - stringLength
							+ (stringLength / 2),
					y + g.getFontMetrics(font).getHeight() - (int)(g.getFontMetrics(font).getHeight() / 2.6d));
			font = originalFont;
	...
	private void drawUnderlay(BufferedImage bi, Graphics2D g, int type, int x, int y, int blurRadius,
			boolean doubleBlur, int expandBlackout)
	{
		BufferedImage blackout = new BufferedImage(getPercentage(CustomCardMaker.cardWidth, getScale()),
				getPercentage(CustomCardMaker.cardHeight, getScale()), type);
	...
	{
		BufferedImage expand = new BufferedImage(getPercentage(CustomCardMaker.cardWidth, getScale()),
				getPercentage(CustomCardMaker.cardHeight, getScale()), BufferedImage.TYPE_INT_ARGB);
		int width = image.getWidth();
		int height = image.getHeight();
		for(int xx = 0; xx < width; xx++)
	...
	private BufferedImage makeTransparent(BufferedImage bi, double percent)
	{
		...
		for(int xx = 0; xx < width; xx++)
		{
			for(int yy = 0; yy < height; yy++)
			{
				...
				if(originalColor.getAlpha() > 0)
				{
					int col = (getPercentage(originalColor.getAlpha(), percent) << 24) | (originalColor.getRed() << 16)
							| (originalColor.getGreen() << 8) | originalColor.getBlue();
					bi.setRGB(xx, yy, col);
				}
			}
		}
	...
	public String getDifferenceXML()
	{
		...
		str += "<textarea name=\"" + replaceNonXMLCharacters(name) + "\" value=\"" + replaceNonXMLCharacters(getValue())
				...
				+ "textsize=\"" + textSize + "\" " + "textsizebold=\"" + textSizeBold + "\" />\n";
		return str;
	}
